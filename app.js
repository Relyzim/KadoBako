  ...
  async function load() {
    ...
    if (mode === "idb") {
      try {
        ...
        try {
          const jr = JSON.parse(localStorage.getItem(JOURNAL_KEY) || "null");
          if (jr && Array.isArray(jr.items) && (!st || (jr.savedAt || 0) > (st.savedAt || 0))) { st = jr; recovered = true; }
        } catch (e) { }
...
const DEFAULT_SETTINGS = {
  baseCurrency: "EUR", usdToEur: 0.92, fxAuto: true, fxDate: null, targetCoef: 1.5, undercutPct: 5, monthlyGoal: 0, view: "list",
...
function normalizeState(s) {
  ...
  const pos = (o, k) => { if (o && typeof o[k] === "number" && !(o[k] >= 0)) o[k] = 0; };
  const fix = (it) => {
    eur(it.acquisition); eur(it.listing); eur(it.sale); if (!Array.isArray(it.tags)) it.tags = [];
    pos(it.acquisition, "purchasePrice"); pos(it.listing, "askingPrice"); pos(it.sale, "soldPrice");
...
function persist() {
  state.settings.baseCurrency = currency; state.savedAt = Date.now();
...
function writeJournal() {
  ...
  try {
    const strip = stripPhotos;
    localStorage.setItem(JOURNAL_KEY, JSON.stringify({ ...state, items: state.items.map(strip), trash: (state.trash || []).map(t => ({ ...t, item: strip(t.item) })) }));
  } catch (e) { }
}
...
async function updateStoreNote() {
  ...
  let extra = "";
  try {
    if (Store.mode !== "memory" && navigator.storage && navigator.storage.estimate) {
      const e = await navigator.storage.estimate();
      if (typeof e.usage === "number") extra = " · " + (e.usage < 1048576
        ? Math.max(1, Math.round(e.usage / 1024)).toLocaleString("fr-FR") + " Ko"
        : (e.usage / 1048576).toLocaleString("fr-FR", { maximumFractionDigits: 1 }) + " Mo") + " utilisés";
    }
  } catch (_) { }
...
function wrapTxt(s, max) {
  const w = String(s || "").split(" "); const out = []; let ln = "";
...
function visualGen(it) {
  return it.type === "sealed"
...
const FALLBACK = {
  ...
  sets: {
    sv: [{ id: "sv03", name: "Flammes Obsidiennes" }, { id: "sv04", name: "Faille Paradoxe" }, { id: "sv05", name: "Forces Temporelles" }],
...
async function loadSets(id) {
  const L = catalogLang, k = L + ":" + id; if (cache.sets[k]) return cache.sets[k]; let sets;
  if (usingFallback && L === "fr") { sets = FALLBACK.sets[id] || []; }
  else {
    try { const d = await jget(tcg(L) + "/series/" + id); sets = (d.sets || []).map(s => ({ id: s.id, name: s.name })); }
...
async function loadCards(setId) {
  const L = catalogLang, k = L + ":" + setId; if (cache.cards[k]) return cache.cards[k]; let cards;
  if (usingFallback && L === "fr") { cards = FALLBACK.cards[setId] || []; }
  else {
    try { const d = await jget(tcg(L) + "/sets/" + setId); cards = (d.cards || []).map(c => ({ id: c.id, localId: c.localId, name: c.name, image: c.image })); }
...
const TP_KEYS = {
  normal: ["normal"], holo: ["holo", "holofoil"], reverse: ["reverse", "reverseHolofoil"],
...
function priceForNet(net, platform) {
  const f = state.settings.fees[platform] || { pct: 0, flat: 0 }; const k = 1 - (f.pct || 0) / 100;
  if (!(net >= 0) || k <= 0) return null; return Math.ceil(((net + (f.flat || 0)) / k) * 100 - 1e-9) / 100;
}
const stockAge = (it) => {
  if (!it.acquisition?.date) return null;
...
function counts() {
  const c = { all: state.items.length, stock: 0, listed: 0, sold: 0 }; state.items.forEach(i => c[i.status]++);
...
function renderGoal() {
  ...
  let main;
  if (goal > 0) {
    const p = Math.max(0, cur) / goal * 100, w = Math.min(100, p);
...
async function initPickers() {
  ...
  $("quickNum").disabled = true; $("quickNum").value = "";
...
$("langPick").addEventListener("click", e => {
  const b = e.target.closest("[data-lang]"); if (!b || b.dataset.lang === catalogLang) return;
...
$("selSerie").addEventListener("change", async e => {
  ...
  set.disabled = true; card.disabled = true; card.innerHTML = `<option value="">—</option>`; $("quickNum").disabled = true; $("quickNum").value = "";
...
$("selSet").addEventListener("change", async e => {
  ...
  $("quickNum").disabled = false; $("quickNum").value = "";
});
// N° rapide : taper « 215 » (ou « 015 ») sélectionne directement la carte du set
const normNum = (s) => String(s || "").trim().toLowerCase().replace(/^0+(?=\w)/, "");
$("quickNum").addEventListener("input", async e => {
  const q = normNum(e.target.value); if (!q) return;
  const cards = await loadCards($("selSet").value), c = cards.find(x => normNum(x.localId) === q);
  e.target.setAttribute("aria-invalid", c ? "false" : "true");
  if (c && $("selCard").value !== c.id) { $("selCard").value = c.id; $("selCard").dispatchEvent(new Event("change")); }
});
...
let entryPhoto = null;
function setEntryPhoto(url) {
  entryPhoto = url || null; const pv = $("inPhotoPv");
...
function clearSelection() {
  selected = null; selMarket = null; entryPhoto = null; setEntryPhoto(null); $("mktBox").hidden = true; $("suggestBox").hidden = true; $("dealBox").hidden = true;
...
function buildNewItem() {
  ...
  const base = {
    id: newId(), quantity: qty, condition: $("selCond").value, grading: null, userPhoto: entryPhoto || null,
  ...
  if (selected.sealed) return {
    ...base, type: "sealed", variant: null, market: null, priceHistory: mr > 0 ? [{ date, ref: mr, manual: true }] : [],
  ...
  return {
    ...base, type: "card", variant: $("selVariant").value || null, market: mkt,
...
function commitAdd(n, into) {
  ...
  if (into) {
    ...
    logEvent(n, `Acheté ×${qtyOf(n)} à ${fmt(n.acquisition.purchasePrice)}`, n.acquisition.date + "T12:00:00");
...
function renderSession() {
  ...
  if (on) {
    $("sessionStats").innerHTML = `<b>${s.count}</b> article${s.count > 1 ? "s" : ""} · <b>${fmt(s.spent)}</b> investis`;
...
// Une session commencée un autre jour est close automatiquement (sinon l'ancienne étiquette continuerait d'être posée)
function closeStaleSession() {
  const s = state.session; if (!s.active || !s.startedAt) return;
  if (localISO(new Date(s.startedAt)) === todayStr()) return;
  const d = new Date(s.startedAt).toLocaleDateString("fr-FR");
  state.session = { active: false, label: "", count: 0, spent: 0 }; persist(); renderSession();
  toast(`Session d'achat du ${d} terminée automatiquement : ${s.count} article${s.count > 1 ? "s" : ""}, ${fmt(s.spent)}`);
}
document.addEventListener("visibilitychange", () => { if (document.visibilityState === "visible" && storeReady) closeStaleSession(); });
function endSession() {
...
function openSoldModal(id) {
  const it = state.items.find(i => i.id === id); const q = qtyOf(it);
...
$("msPlatform").addEventListener("change", e => {
  $("msCustomWrap").hidden = e.target.value !== "__custom";
...
$("msConfirm").addEventListener("click", () => {
  ...
  const sale = {
    soldPrice: price, currency, date: todayStr(), platform: readPlatform("msPlatform", "msCustom"),
...
function openEditModal(id) {
  ...
  toggleEditFields(it.status); $("modalEdit").classList.add("on"); editSnapshot = editState();
}
...
$("meConfirm").addEventListener("click", () => {
  ...
  if (mr !== before.mr) {
    it.manualRef = mr > 0 ? mr : null; it.manualRefDate = mr > 0 ? todayStr() : null;
    if (mr > 0) {
      it.priceHistory = it.priceHistory || []; const h = it.priceHistory.find(x => x.date === todayStr());
  ...
  if (ns === "stock") { it.sale = null; }
    ...
    const sp = parseFloat($("meSoldPrice").value); if (!(sp >= 0)) { toast("Renseigne le prix réel"); return; }
    it.sale = {
      soldPrice: sp, currency, date: it.sale?.date || todayStr(), platform: readPlatform("meSoldPlatform", "meSoldCustom"),
  ...
  it.status = ns; it.updatedAt = new Date().toISOString(); persist(); closeModals(true); renderAll(); toast("Modifications enregistrées");
});
...
$("meDuplicate").addEventListener("click", () => {
  ...
  if (editDirty() && !confirm("Tes modifications non enregistrées ne seront pas copiées. Dupliquer quand même ?")) return;
  const c = JSON.parse(JSON.stringify(src));
  Object.assign(c, {
    id: newId(), quantity: 1, status: "stock", sale: null, userPhoto: null, userPhoto2: null, log: [],
  ...
  state.items.push(c); persist(); renderAll(); closeModals(true);
...
function parseTags(str) {
  ...
  for (const raw of String(str || "").split(",")) {
    const t = raw.trim().slice(0, 30); const k = t.toLowerCase();
...
$("meTagPicks").addEventListener("click", e => {
...
function dropPrice(id) {
  ...
  it.listing.askingPrice = nw; logEvent(it, `Baisse de prix −10 % : ${fmt(old)} → ${fmt(nw)}`); const entry = it.log[it.log.length - 1];
  persist(); renderAll();
  toast(`Prix baissé : ${fmt(old)} → ${fmt(nw)}`, { action: "Annuler", onAction: () => { it.listing.askingPrice = old; it.log = it.log.filter(x => x !== entry); persist(); renderAll(); } });
}
...
function renderTrash() {
  ...
  state.trash.forEach(t => {
    const it = t.item, left = Math.max(0, TRASH_DAYS - Math.floor((Date.now() - new Date(t.deletedAt)) / 86400000));
...
$("waConfirm").addEventListener("click", () => {
  ...
  else state.wishlist.push({
    id: newId("w_"), cardId: selected.id, name: selected.name, number: selected.number, set: selected.set,
...
function renderWish() {
  ...
  state.wishlist.forEach(w => {
    const hit = wishHit(w), r = document.createElement("div"); r.className = "row-mini" + (hit ? " hit" : "");
    r.innerHTML = `<div class="thumb"><img alt=""></div>
...
$("wishList").addEventListener("change", e => {
  const inp = e.target.closest("[data-wid]"); if (!inp) return;
...
/* ---------- Scanner : cadre de visée, empreinte (formes + couleurs), 3 meilleures correspondances ---------- */
let camStream = null, barcodeDetector = null;
try { if ("BarcodeDetector" in window) barcodeDetector = new BarcodeDetector(); } catch (e) { }
const SIG_CACHE = new Map();                                    // url de l'image catalogue -> empreinte
// Empreinte : dHash 9×8 (formes, insensible à la luminosité) + teintes d'une grille 4×4 (couleurs, normalisées)
function signature(src, sx, sy, sw, sh) {
  const g = document.createElement("canvas"); g.width = 9; g.height = 8; const gx = g.getContext("2d", { willReadFrequently: true });
  gx.drawImage(src, sx, sy, sw, sh, 0, 0, 9, 8); const d = gx.getImageData(0, 0, 9, 8).data;   // lève une SecurityError si l'image est protégée
  const gray = []; for (let i = 0; i < 72; i++) gray.push(0.299 * d[i * 4] + 0.587 * d[i * 4 + 1] + 0.114 * d[i * 4 + 2]);
  let bits = ""; for (let y = 0; y < 8; y++) for (let x = 0; x < 8; x++) bits += gray[y * 9 + x] < gray[y * 9 + x + 1] ? "0" : "1";
  const c = document.createElement("canvas"); c.width = 4; c.height = 4; const cx = c.getContext("2d", { willReadFrequently: true });
  cx.drawImage(src, sx, sy, sw, sh, 0, 0, 4, 4); const e = cx.getImageData(0, 0, 4, 4).data, chroma = [];
  for (let i = 0; i < 16; i++) { const s = e[i * 4] + e[i * 4 + 1] + e[i * 4 + 2] + 1; chroma.push(e[i * 4] / s, e[i * 4 + 1] / s); }
  // Vignette 16×16 centrée-réduite : corrélation insensible à la luminosité, au contraste et au bruit
  const t = document.createElement("canvas"); t.width = 16; t.height = 16; const tx = t.getContext("2d", { willReadFrequently: true });
  tx.drawImage(src, sx, sy, sw, sh, 0, 0, 16, 16); const f = tx.getImageData(0, 0, 16, 16).data, lum = [];
  for (let i = 0; i < 256; i++) lum.push(0.299 * f[i * 4] + 0.587 * f[i * 4 + 1] + 0.114 * f[i * 4 + 2]);
  const mean = lum.reduce((a, b) => a + b, 0) / 256, sd = Math.sqrt(lum.reduce((a, b) => a + (b - mean) ** 2, 0) / 256) || 1;
  return { bits, chroma, norm: lum.map(v => (v - mean) / sd) };
}
function sigDistance(a, b) {                                     // 0 = identique, 1 = rien à voir
  let h = 0; for (let i = 0; i < 64; i++) if (a.bits[i] !== b.bits[i]) h++;
  let c = 0; for (let i = 0; i < a.chroma.length; i++) c += Math.abs(a.chroma[i] - b.chroma[i]);
  let r = 0; for (let i = 0; i < 256; i++) r += a.norm[i] * b.norm[i]; r /= 256;             // corrélation : 1 = même image
  return 0.5 * (1 - r) / 2 + 0.2 * (h / 64) + 0.3 * Math.min(1, c / a.chroma.length * 6);
}
function imageSignature(url) {
  if (SIG_CACHE.has(url)) return Promise.resolve(SIG_CACHE.get(url));
  return new Promise((res, rej) => {
    const im = new Image(); im.crossOrigin = "anonymous";
    im.onload = () => { try { const s = signature(im, 0, 0, im.naturalWidth, im.naturalHeight); SIG_CACHE.set(url, s); res(s); } catch (e) { rej(Object.assign(new Error("cors"), { cors: true })); } };
    im.onerror = () => rej(new Error("réseau")); im.src = url;
  });
}
// Zone de la vidéo qui correspond exactement au cadre affiché (la vidéo est recadrée pour remplir l'écran)
function guideRect(v) {
  const r = v.getBoundingClientRect(), vw = v.videoWidth, vh = v.videoHeight, s = Math.max(r.width / vw, r.height / vh);
  let gh = 0.8 * r.height, gw = gh * 63 / 88; if (gw > 0.9 * r.width) { gw = 0.9 * r.width; gh = gw * 88 / 63; }
  const visW = r.width / s, visH = r.height / s;
  return { sx: (vw - visW) / 2 + (r.width - gw) / 2 / s, sy: (vh - visH) / 2 + (r.height - gh) / 2 / s, sw: gw / s, sh: gh / s };
}
$("btnScan").addEventListener("click", async () => {
  $("modalScan").classList.add("on"); $("scCands").innerHTML = ""; $("camView").style.display = "";
  $("scNote").textContent = $("selSet").value ? "" : "Choisis d'abord une série et un set : la reconnaissance se fait parmi ses cartes.";
  try { camStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 1280 } } }); $("camView").srcObject = camStream; await $("camView").play(); }
  catch (e) { $("scNote").textContent = "Caméra indisponible. Autorise l'accès à la caméra, ou utilise le n° rapide."; $("camView").style.display = "none"; }
});
$("scCapture").addEventListener("click", async () => {
  const v = $("camView"), note = $("scNote"), box = $("scCands"); box.innerHTML = "";
  if (!v.videoWidth) { note.textContent = "Pas d'image de la caméra."; return; }
  const setId = $("selSet").value; if (!setId) { note.textContent = "Choisis d'abord une série et un set."; return; }
  const g = guideRect(v), frame = document.createElement("canvas"); frame.width = v.videoWidth; frame.height = v.videoHeight;
  frame.getContext("2d").drawImage(v, 0, 0);
  let codeTxt = ""; if (barcodeDetector) { try { const codes = await barcodeDetector.detect(frame); if (codes.length) codeTxt = ` Code-barres lu : ${codes[0].rawValue}.`; } catch (e) { } }
  const target = signature(frame, g.sx, g.sy, g.sw, g.sh);
  const cards = (await loadCards(setId)).filter(c => c.image);
  if (!cards.length) { note.textContent = "Pas d'images pour ce set : utilise le n° rapide." + codeTxt; return; }
  let done = 0, cors = 0, failed = 0; const scored = [], queue = [...cards];
  note.textContent = `Comparaison avec les cartes du set… 0/${cards.length}`;
  await Promise.all(Array.from({ length: 8 }, async () => {
    while (queue.length) {
      const c = queue.shift();
      try { const s = await imageSignature(img(c.image, "low")); scored.push({ c, d: sigDistance(target, s) }); }
      catch (e) { if (e.cors) cors++; else failed++; }
      note.textContent = `Comparaison avec les cartes du set… ${++done}/${cards.length}`;
    }
  }));
  if (!scored.length) {
    note.textContent = (cors ? "Le site des images TCGdex n'autorise pas leur analyse depuis cette adresse. Utilise le n° rapide : la reconnaissance fonctionnera une fois l'app hébergée sur ton NAS."
      : "Images du catalogue indisponibles (hors-ligne ?). Utilise le n° rapide.") + codeTxt; return;
  }
  scored.sort((a, b) => a.d - b.d);
  const best = scored.slice(0, 3), sure = best[0].d < 0.35 && (best.length < 2 || best[1].d - best[0].d >= 0.04);
  note.textContent = (sure ? "Carte reconnue : confirme d'un tap." : "Correspondance incertaine : vérifie avant de choisir.") + codeTxt;
  box.innerHTML = best.map(({ c, d }) => `<button type="button" class="cand" data-card="${esc(c.id)}"><img alt="" src="${esc(img(c.image, "low"))}"><b>${esc(c.name)}</b><span>n°${esc(c.localId)} · ${Math.round((1 - d) * 100)} %</span></button>`).join("");
});
$("scCands").addEventListener("click", e => {
  const b = e.target.closest("[data-card]"); if (!b) return;
  $("selCard").value = b.dataset.card; $("selCard").dispatchEvent(new Event("change")); closeModals(true); toast("Carte sélectionnée : vérifie puis indique le prix");
});
...
function renderTreso() {
  ...
  state.items.forEach(it => {
    const q = qtyOf(it), pa = it.acquisition.purchasePrice || 0; out += pa * q;
  ...
  state.items.forEach(it => {
    const q = qtyOf(it), a = idx[monthOf(it.acquisition?.date)]; if (a != null) months[a].out += (it.acquisition.purchasePrice || 0) * q;
  ...
  const bars = months.map((m, i) => {
    const x = P + i * bw, h1 = m.out / max * (H - 2 * P), h2 = m.inn / max * (H - 2 * P);
    return `<rect x="${(x + bw * 0.18).toFixed(1)}" y="${(H - P - h1).toFixed(1)}" width="${(bw * 0.3).toFixed(1)}" height="${h1.toFixed(1)}" rx="3" style="fill:var(--pink)"><title>${m.lbl} : ${fmt(m.out)} sortis</title></rect>
      ...
      <text x="${(x + bw / 2).toFixed(1)}" y="${H - 6}" text-anchor="middle" font-size="11" style="fill:var(--muted)">${m.lbl}</text>`;
  }).join("");
...
function openAnalytics() {
  ...
  const evts = []; state.items.forEach(it => {
    const q = qtyOf(it);
  ...
  // agrégations
  const grp = (keyFn) => {
    const m = {}; sold.forEach(it => { const k = keyFn(it) || "—"; (m[k] = m[k] || { sum: 0, n: 0 }).sum += realMargin(it); m[k].n++; });
...
$("anYearCsv").addEventListener("click", () => {
  ...
  const rows = sales.map(i => {
    const q = qtyOf(i), ca = (i.sale.soldPrice || 0) * q, fp = i.sale.fees?.platformFee || 0, fe = i.sale.fees?.shipping || 0, co = (i.acquisition.purchasePrice || 0) * q, m = realMargin(i);
...
/* ---------- Taux dollar → euro automatique (taux BCE publiés chaque jour ouvré) ---------- */
async function updateFx(force) {
  const s = state.settings; if (s.fxAuto === false) return false;
  if (!force && s.fxDate && Date.now() - new Date(s.fxDate).getTime() < 12 * 3600000) return false;
  const get = async (url, pick) => {
    const ctl = new AbortController(), t = setTimeout(() => ctl.abort(), 5000);
    try { const r = await fetch(url, { signal: ctl.signal }); if (!r.ok) throw new Error(r.status); return pick(await r.json()); } finally { clearTimeout(t); }
  };
  let rate = null;
  try { rate = await get("https://api.frankfurter.dev/v2/rate/USD/EUR", d => d && d.rate); } catch (e) { }
  if (!(rate > 0)) try { rate = await get("https://api.frankfurter.app/latest?from=USD&to=EUR", d => d && d.rates && d.rates.EUR); } catch (e) { }
  if (!(rate > 0.3 && rate < 3)) return false;                        // réponse absurde : on garde l'ancien taux
  s.usdToEur = Math.round(rate * 10000) / 10000; s.fxDate = new Date().toISOString(); persist(); return true;
}
function renderFxInfo() {
  const s = state.settings, auto = $("stFxAuto").checked; $("stFx").disabled = auto;
  $("stFxInfo").textContent = auto
    ? (s.fxDate ? `Taux BCE mis à jour le ${new Date(s.fxDate).toLocaleDateString("fr-FR")} : 1 $ = ${String(s.usdToEur).replace(".", ",")} €` : "Taux par défaut : il sera mis à jour dès que tu seras en ligne.")
    : "Taux fixé à la main : pense à l'actualiser de temps en temps.";
}
$("stFxAuto").addEventListener("change", async () => {
  renderFxInfo();
  if ($("stFxAuto").checked) { state.settings.fxAuto = true; if (await updateFx(true)) { $("stFx").value = state.settings.usdToEur; renderFxInfo(); } }
});
...
$("stConfirm").addEventListener("click", () => {
  ...
  s.targetCoef = parseFloat($("stCoef").value) || 1.5; s.undercutPct = parseFloat($("stUndercut").value) || 0; s.fxAuto = $("stFxAuto").checked; if (!s.fxAuto) s.usdToEur = parseFloat($("stFx").value) || 0.92;
...
/* ---------- Modales fermeture ---------- */
// Instantané de la fiche d'édition à l'ouverture, pour détecter les modifications non enregistrées
let editSnapshot = null;
const editState = () => [...$("modalEdit").querySelectorAll("input:not([type=file]),select,textarea")].map(e => e.value).join("\u0001") + "|" + (editPhoto || "").length + "|" + (editPhoto2 || "").length;
const editDirty = () => $("modalEdit").classList.contains("on") && editSnapshot !== null && editState() !== editSnapshot;
// force === true uniquement (les boutons « Fermer » transmettent un événement, qui ne doit pas forcer)
function closeModals(force) {
  if (force !== true && editDirty() && !confirm("Fermer sans enregistrer tes modifications ?")) return false;
  editSnapshot = null;
  document.querySelectorAll(".scrim").forEach(s => s.classList.remove("on"));
...
function downloadBlob(blob, name) {
  if (IS_IOS && navigator.canShare) {            // iOS : un téléchargement dans l'app installée ouvre une vue sans retour possible
    try {
      ...
      if (navigator.canShare({ files: [f] })) return navigator.share({ files: [f], title: name }).then(() => true, () => false);
    } catch (e) { }
  }
...
/* ---------- Import : vérifié, confirmé, annulable ---------- */
const numOk = (v) => typeof v === "number" && isFinite(v) && v >= 0;
function validItem(it) {
  return it && typeof it === "object" && typeof it.id === "string" && it.catalog && typeof it.catalog.name === "string" && it.catalog.set && typeof it.catalog.set === "object"
    && it.acquisition && numOk(it.acquisition.purchasePrice) && ["stock", "listed", "sold"].includes(it.status)
    && (it.status !== "sold" || (it.sale && numOk(it.sale.soldPrice)));
}
...
$("fileImport").addEventListener("change", e => {
  ...
  r.onload = () => {
    let d; try { d = JSON.parse(r.result); } catch (err) { toast("Fichier illisible : ce n'est pas un fichier JSON"); return; }
    if (!d || typeof d !== "object" || !Array.isArray(d.items)) { toast("Ce fichier n'est pas une sauvegarde KadoBako"); return; }
    const good = d.items.filter(validItem), bad = d.items.length - good.length;
    if (!good.length && d.items.length) { toast("Sauvegarde inutilisable : aucune ligne lisible"); return; }
    d.items = good; if (Array.isArray(d.trash)) d.trash = d.trash.filter(t => t && validItem(t.item));
    if (Array.isArray(d.wishlist)) d.wishlist = d.wishlist.filter(w => w && typeof w.cardId === "string" && numOk(w.targetPrice));
    pendingImport = d;
    const dates = good.map(i => i.updatedAt || i.createdAt).filter(Boolean).sort(), last = dates.length ? new Date(dates[dates.length - 1]).toLocaleDateString("fr-FR") : "inconnue";
    $("imSub").textContent = `Fichier « ${f.name} »`;
    $("imCalc").innerHTML = `<div><span>Dans le fichier</span><span>${good.length} ligne${good.length > 1 ? "s" : ""} · dernière modification le ${last}</span></div>
      <div><span>Actuellement dans l'app</span><span>${state.items.length} ligne${state.items.length > 1 ? "s" : ""}</span></div>`;
    $("imWarn").textContent = (bad ? `⚠ ${bad} ligne${bad > 1 ? "s" : ""} illisible${bad > 1 ? "s" : ""} sera ignorée${bad > 1 ? "s" : ""}. ` : "")
      + (state.items.length ? "Tes données actuelles seront remplacées, mais restent récupérables 30 jours dans la corbeille." : "");
    $("modalImport").classList.add("on");
  };
  r.readAsText(f);
});
$("imConfirm").addEventListener("click", () => {
  if (!pendingImport) return;
  const prev = JSON.parse(JSON.stringify(state)), now = new Date().toISOString();
  const next = normalizeState(pendingImport); pendingImport = null;
  next.trash = [...state.items.map(item => ({ item, deletedAt: now })), ...next.trash];   // l'existant n'est jamais perdu
  state = next; persist(); closeModals(true); renderAll(); renderSession();
  toast(`${state.items.length} lignes importées`, { action: "Annuler", onAction: () => { state = normalizeState(prev); persist(); renderAll(); renderSession(); toast("Import annulé"); } });
});
$("btnClear").addEventListener("click", () => {
  if (!state.items.length) { toast("Rien à vider"); return; }
  ...
  persist(); renderAll();
  toast(`${n} lignes déplacées dans la corbeille`, {
...
/* ---------- Champs numériques : jamais sous leur minimum (pas de prix négatif, pas de quantité 0) ---------- */
// Touches « − », « + » et « e » bloquées dans les champs sans valeur négative (le clavier numérique iOS ne les propose pas)
document.addEventListener("keydown", e => {
  const t = e.target; if (t instanceof HTMLInputElement && t.type === "number" && t.min !== "" && parseFloat(t.min) >= 0 && ["-", "+", "e", "E"].includes(e.key)) e.preventDefault();
}, true);
// Valeur collée ou saisie sous le minimum : ramenée au minimum
document.addEventListener("input", e => {
  const t = e.target; if (!(t instanceof HTMLInputElement) || t.type !== "number" || t.min === "" || t.value === "") return;
  const v = parseFloat(t.value), min = parseFloat(t.min); if (isFinite(v) && v < min) t.value = t.min;
}, true);   // phase de capture : corrigé avant tout calcul
...
let modalTrigger = null, lastTouched = null;
// Visible ? (le contenu d'un menu refermé garde une boîte dans Chrome : on teste explicitement le menu fermé)
const isShown = (el) => !!el && document.contains(el) && !el.closest("details:not([open]) > :not(summary)")
  && (el.checkVisibility ? el.checkVisibility() : !!el.offsetParent);
// Dernier élément touché hors fenêtre : sert de repli quand le bouton d'origine a disparu (menu refermé)
// ou n'a pas reçu le focus (Safari ne donne pas le focus aux boutons touchés)
document.addEventListener("pointerdown", e => { const b = e.target.closest("button,summary,select,input,[tabindex]"); if (b && !b.closest(".scrim")) lastTouched = b; }, true);
document.addEventListener("focusin", e => { if (!e.target.closest(".scrim")) lastTouched = e.target; });
function syncModalA11y() {
  const open = document.querySelector(".scrim.on"), wrap = document.querySelector(".wrap");
  document.documentElement.classList.toggle("modal-open", !!open);
  if (open) {
    if (!wrap.inert) {
      const a = document.activeElement;           // mémorise le bouton qui a ouvert la fenêtre
      modalTrigger = (a && a !== document.body && !a.closest(".scrim")) ? a : lastTouched;
    }
    wrap.inert = true; document.querySelectorAll(".scrim").forEach(s => { s.inert = s !== open; });
    const m = open.querySelector(".modal");
    if (m && !m.contains(document.activeElement)) { m.setAttribute("tabindex", "-1"); m.focus({ preventScroll: true }); }   // pas de clavier ouvert d'office sur mobile
  } else if (wrap.inert) {
    wrap.inert = false; document.querySelectorAll(".scrim").forEach(s => { s.inert = false; });
    let t = modalTrigger; modalTrigger = null;
    if (t && document.contains(t) && !isShown(t)) t = t.closest("details")?.querySelector("summary") || null;   // bouton d'un menu refermé
    if (isShown(t)) t.focus({ preventScroll: true });
  }
}
new MutationObserver(syncModalA11y).observe(document.body, { subtree: true, attributes: true, attributeFilter: ["class"] });
// Tabulation bouclée dans la fenêtre (navigateurs sans « inert »)
document.addEventListener("keydown", e => {
  if (e.key !== "Tab") return; const m = document.querySelector(".scrim.on .modal"); if (!m) return;
  const f = [...m.querySelectorAll('button,input,select,textarea,summary,[href],[tabindex]:not([tabindex="-1"])')].filter(x => !x.disabled && x.offsetParent);
...
(async () => {
  ...
  updateFx();                              // en arrière-plan, sans bloquer le démarrage
  closeStaleSession();                     // en dernier : son message ne doit pas être masqué par un autre
